...
public class MapTestScenarios {
  ...
  public MapTestScenarios(
      MapTestScenariosPromptGenerator promptGenerator, B3GPTGateway b3GPTGateway) {
    this.promptGenerator = promptGenerator;
    this.b3GPTGateway = b3GPTGateway;
  }

  /**
   * Extracts methods/dependencies from the analysis, generates a prompt, calls the IA to map test
   * scenarios, and returns the result.
   *
  ...
  public AssistantMessage run(
      AssistantMessage analyzedClass, String targetClassName, String targetClassCode) {
    ...
    String publicMethodsSection =
        extractSection(analysisText, PUBLIC_METHODS_MARKER, DEPENDENCIES_MARKER);
    String dependenciesSection =
        extractSection(
            analysisText, DEPENDENCIES_MARKER, null); // Goes until the end or next major section

    log.info("Extracted Public Methods:\n{}", publicMethodsSection);
    log.info("Extracted Dependencies:\n{}", dependenciesSection);

    final Prompt prompt =
        this.promptGenerator.get(
            targetClassName, targetClassCode, publicMethodsSection, dependenciesSection);
