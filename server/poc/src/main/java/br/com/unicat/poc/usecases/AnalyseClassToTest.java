...
public class AnalyseClassToTest {
  ...
  public AssistantMessage run(
      ...
      final String targetClassToTest,
      final String targetClassPackage) {
    log.info(
        "INIT run AnalyseAndMapClassUnitTestScenarios. target class: {}, target package: {}",
        targetClassName,
        targetClassPackage);
    final Prompt prompt =
        this.promptGenerator.get(targetClassName, targetClassPackage, targetClassToTest);
    ChatResponse response = this.b3gptGateway.callAPI(prompt);

    log.info(
        "END AnalyseAndMapClassUnitTestScenarios run. response: \n{}",
